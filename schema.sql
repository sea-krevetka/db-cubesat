...
-- Авторы: Команда из 2 человек
-- Версия: 1.0
-- =====================================================

-- Удаляем старые объекты (для чистой инициализации)
...
COMMENT ON TABLE satellites IS 'Информация о спутниках группировки';
COMMENT ON COLUMN satellites.status IS 'operational, maintenance, safe_mode, decommissioned';

-- =====================================================
-- 2. Подсистемы спутников
-- =====================================================
...
COMMENT ON TABLE subsystems IS 'Подсистемы спутника (ЭП, БКУ, СВ, etc.)';
...
COMMENT ON TABLE sensors IS 'Датчики телеметрии';
COMMENT ON COLUMN sensors.sensor_type IS 'temperature, voltage, current, orientation, radiation';

-- =====================================================
-- 4. Телеметрия (основная толстая таблица)
-- =====================================================
...
-- Индекс для поиска по датчику + времени
CREATE INDEX idx_telemetry_sensor_time ON telemetry(sensor_id, timestamp DESC);
-- Индекс для агрегаций по спутникам (через сенсоры)
CREATE INDEX idx_telemetry_sensor ON telemetry(sensor_id);

COMMENT ON TABLE telemetry IS 'Поток телеметрии (значения с датчиков)';
...
CREATE TABLE planned_passes (
    ...
    operator_id INTEGER,  -- ID оператора (можно связать с таблицей users, если будет)
...
-- =====================================================
-- Выдача прав (для production — настроить отдельно)
-- =====================================================
